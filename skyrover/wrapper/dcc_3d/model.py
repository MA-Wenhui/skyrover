...
class CommLayer(nn.Module):
    ...
    def position_embed(self, relative_pos, dtype, device):
        ...
        relative_pos += 4
        relative_pos = relative_pos.reshape(batch_size * num_agents * num_agents, 3)
...
class Network(nn.Module):
    def __init__(
        ...
        self.latent_dim = self.hidden_dim + config.action_dim
        self.obs_shape = input_shape
        self.selective_comm = selective_comm

        self.obs_encoder = nn.Sequential(
            nn.Conv3d(input_shape[0], 64, kernel_size=3, stride=1,padding=1),  # 输入通道为3，保持所有三个维度的感受野
            nn.LeakyReLU(0.2, True),
            nn.Conv3d(64, 64, kernel_size=3, stride=1),
            nn.LeakyReLU(0.2, True),
            nn.Conv3d(64, 128, kernel_size=3, stride=1),
            nn.LeakyReLU(0.2, True),
            nn.Conv3d(128, 192, kernel_size=3, stride=1),
            nn.LeakyReLU(0.2, True),
            nn.Conv3d(192, 256, kernel_size=3, stride=1),
            nn.LeakyReLU(0.2, True),
            nn.Flatten(),
        )
        ...
        # dueling q structure
        self.adv = nn.Linear(self.hidden_dim, config.action_dim)
    ...
    def step(self, obs, last_act, pos):
        ...
        # in_obs_mask = torch.ones((num_agents, num_agents), dtype=torch.bool)
        in_obs_mask[agent_indexing, agent_indexing] = 0  

        if self.selective_comm:
            # 对a_i，当前obs_i内，如果存在a_j，将obs_i中a_j位置信息抠除，
            # 得到obs_i_-j，将obs_i和obs_i_-j分别输入model，得到a_i和a_i_-j
            # 如果a_i和a_i_-j不一致，则a_i和a_j通信
            ...
            )[test_mask] # size: (5,5,4,9,9)[5,5] -> (k, 4, 9, 9), k: True num in test_mask
